...
if($job=='addsp'&&!$web_admin)
{
	...
	$rs=$db->get_one("SELECT COUNT(*) AS NUM FROM `{$_pre}special` WHERE uid='$lfjuid'");
...
if($job=='listsp')
{
	...
	$min=($page-1)*$rows;
	$showpage=getpage("{$_pre}special","WHERE uid='$lfjuid'","?job=listsp",$rows);
	$query = $db->query("SELECT S.*,F.name AS fname FROM {$_pre}special S LEFT JOIN {$_pre}spsort F ON S.fid=F.fid WHERE S.uid='$lfjuid' ORDER BY id DESC LIMIT $min,$rows");
	while($rs = $db->fetch_array($query)){
		...
		if($rs[yz]){
			$rs[_yz]="<img alt='已审核' src='$webdb[www_url]/member/images/check_yes.gif'>";
		}else{
			$rs[_yz]="<img alt='未审核' src='$webdb[www_url]/member/images/check_no.gif'></a>";
		}
		$listdb[]=$rs;
	}
...
elseif($job=="editsp")
{
	...
	if($step==2){
		...
		$db->query("UPDATE {$_pre}special SET title='$postdb[title]',fid='$postdb[fid]',picurl='$postdb[picurl]',content='$postdb[content]',allowpost='$postdb[allowpost]',banner='$postdb[banner]' WHERE uid=$lfjuid AND id='$id'");
...
elseif($job=='delsp')
{
	$rsdb=$db->get_one("SELECT * FROM {$_pre}special WHERE uid='$lfjuid' AND id='$id'");
	delete_attachment($lfjdb[uid],$rsdb[picurl]);
	$db->query("DELETE FROM {$_pre}special WHERE uid=$lfjuid AND id='$id'");
...
elseif($job=="show_iframe"){
	...
	if($type=="list_atc"||$type=='all')
	{
		...
		while($rs = $db->fetch_array($query)){
			$rs[subject]="<a href='$Mdomain/bencandy.php?fid=$rs[fid]&id=$rs[aid]' target=_blank>$rs[title]</a>";
			$_listdb[$rs[aid]]=$rs;
		}
	...
	if($type=='myatc'||$type=='all')
	{
		...
		while($rs = $db->fetch_array($query)){
			...
			$show.="<tr align='center' class='trA' onmouseover=\"this.className='trB'\" onmouseout=\"this.className='trA'\"> 
                ...
                <td width='84%' align='left'><A HREF='$Mdomain/bencandy.php?fid=$rs[fid]&id=$rs[aid]' target=_blank>{$rs[title]}</A></td>
                <td width='11%'>&nbsp;$add</td>
              </tr>";
		}
		$show="<table width='100%' border='0' cellspacing='1' cellpadding='3'>
		...
		$sort_fid=str_replace("<select name='fid'","[<A target='spiframe'  HREF='special.php?job=show_iframe&id=$id&type=myatc&fid=$fid&ismy=1' style='color:$color_me;'>我的软件</A>] [<A target='spiframe'  HREF='special.php?job=show_iframe&id=$id&type=myatc&fid=$fid' style='color:$color_all;'>所有软件</A>]<select onChange='fid_jumpMenu(this)'",$sort_fid);
		echo "<SCRIPT LANGUAGE='JavaScript'>
		<!--
