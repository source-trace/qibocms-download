...
if(!is_writable(Mpath."cache/makeShow1.php")){
	showerr(Mpath."cache/makeShow1.php文件不存在,或文件不可写");
}
...
if($rsdb[aids])
{
	$query = $db->query("SELECT * FROM {$_pre}article WHERE aid IN ($rsdb[aids])");
...
if(!$listdb){
	$listdb[]=array("subject"=>"本专题暂无软件,请管理员在后台添加软件");
}
...
$ch_module = $webdb[module_id];										//软件模块,默认为0
$ch = 0;											//不属于任何专题
...
if(!$rsdb[yz]){
	$showsp="<META HTTP-EQUIV=REFRESH CONTENT='0;URL=$Mdomain/showsp.php?fid=$fid&id=$id'>";
}else{
	$showsp="";
}
...
if($string=$iddb[++$II]){
	$ar=explode("-",$string);
	write_file(Mpath."cache/makeShow_record.php","?fid=$ar[0]&id=$ar[1]&II=$II");
	echo '<meta http-equiv="Content-Type" content="text/html; charset=gb2312" />';
	echo "请稍候,正在生成专题内容页静态...<META HTTP-EQUIV=REFRESH CONTENT='0;URL=?fid=$ar[0]&id=$ar[1]&II=$II'>";
	exit;
}else{
	unlink(Mpath."cache/makeShow1.php");
	unlink(Mpath."cache/makeShow_record.php");
	if(count($iddb)==1){
