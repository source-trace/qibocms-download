...
EOT;
	...
	$query = $db->query("SELECT * FROM {$_pre}article$erp WHERE fid='$fid' AND yz=1 ORDER BY aid DESC LIMIT 30");
	while($rs = $db->fetch_array($query)){
...
<item>
 ...
 <link><![CDATA[{$Mdomain}/bencandy.php?fid={$rs[fid]}&id={$rs[aid]}]]></link>
...
elseif($fid&&$id){
	...
	$erp=$Fid_db[iftable][$fid];
	$rsdb=$db->get_one("SELECT A.*,R.content FROM {$_pre}article$erp A LEFT JOIN {$_pre}reply R ON A.aid=R.aid WHERE A.aid='$id' AND A.yz=1 LIMIT 1");
	//check_article($rsdb);
...
<channel>
 ...
 <link><![CDATA[{$Mdomain}/bencandy.php?fid=$fid&id=$id]]></link>
...
EOT;
	...
	$query = $db->query("SELECT * FROM {$_pre}reply WHERE aid='$id' AND topic=0 ORDER BY rid DESC LIMIT 30");
	while($rs = $db->fetch_array($query)){
