...
if($action=="post"){
	...
	$rss=$db->get_one("SELECT A.*,B.allowcomment AS Fallowcomment FROM {$_pre}article A LEFT JOIN {$_pre}sort B ON A.fid=B.fid WHERE A.aid='$aid'");
	if(!$rss){
	...
	if($allow)
	{
		...
		extract($db->get_one("SELECT COUNT(*) AS NUM FROM `{$_pre}comment` WHERE $SQL"));
		if($NUM>$groupdb[comment_num]){
			die("24小时内发布的评论总数不能超过{$groupdb[comment_num]}条");
		}
		$db->query("INSERT INTO `{$_pre}comment` (`aid` , `fid` , `uid` , `username` , `posttime` , `content` , `ip` , `icon` , `yz` ,`authorid`) VALUES ('$aid', '$fid', '$lfjuid', '$username', '$timestamp', '$content', '$onlineip', '$commentface', '$yz','$rss[uid]')");
		$db->query("UPDATE {$_pre}article SET comments=comments+1 WHERE aid='$aid' ");
	}
}
...
elseif($action=="del")
{
	$rs=$db->get_one("SELECT * FROM `{$_pre}comment` WHERE cid='$cid'");
	if($web_admin||($lfjuid&&$lfjuid==$rs[uid]) )
	{
		$db->query("DELETE FROM `{$_pre}comment` WHERE cid='$cid'");
...
{
	if($action=="uncom"){
		$db->query("UPDATE {$_pre}comment SET ifcom=0 WHERE cid='$cid' ");
	}else{
		$db->query("UPDATE {$_pre}comment SET ifcom=1 WHERE cid='$cid' ");
	}
}
...
elseif($action=='vote')
{
	...
	if($job=='agree')
	{
		...
		else
		{
			...
			$db->query("UPDATE {$_pre}comment SET agree=agree+1 WHERE cid='$rs[cid]' ");
		}
	}
	elseif($job=='disagree')
	{
		...
		{
			set_cookie("agree_$cid",1,3600);
			$db->query("UPDATE {$_pre}comment SET disagree=disagree+1 WHERE cid='$rs[cid]' ");
		}
	}
	if($posttype!='ajax')
	{
...
$query=$db->query("SELECT SQL_CALC_FOUND_ROWS A.*,B.icon,A.icon AS img FROM `{$_pre}comment` A LEFT JOIN {$pre}memberdata B ON A.uid=B.uid WHERE A.aid=$aid $SQL ORDER BY $orderSQL A.cid DESC LIMIT $min,$rows");
...
$showpage=getpage('','',"?fid=$fid&aid=$aid",$rows,$totalNum);
if($iframeID){
	$showpage=str_replace("href=\"?","target=\"$iframeID\" href=\"$Mdomain/comment_ajax.php?iframeID=$iframeID&",$showpage);
}else{
	$showpage=preg_replace("/\?fid=([\d]+)&aid=([\d]+)&page=([\d]+)/is","javascript:getcomment('$Mdomain/comment_ajax.php?fid=\\1&aid=\\2&page=\\3')",$showpage);
}
