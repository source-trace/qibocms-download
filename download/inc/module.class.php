...
function Module_Field($path=''){
	...
	$this->table_title = $_pre.'article';
	$this->table_content = $_pre."reply";
	...
	$this->tpl_admin_new = $this->f_path."admin/template/post/";
	$this->tpl_member_new = $this->f_path."member/template/";
}
...
function delete_module($id){
	...
	$db->query("DROP TABLE IF EXISTS `{$this->table_content}`");
...
function make_template($id){
	...
	$adminpost_tpl = $tplDB->read_tpl("post_0.htm",$tpldb['admin_post'],'admin');
	$memberpost_tpl = $tplDB->read_tpl("post_0.htm",$tpldb['member_post'],'member');


	$indexshow_tpl = $tplDB->read_tpl("bencandy_0.htm",$tpldb['show'],'index');
	//$indexpost_tpl = $tplDB->read_tpl("post_0.htm",$tpldb['post'],'index');
	//$indexsearch_tpl = $tplDB->read_tpl("search_0.htm",$tpldb['search'],'index');	
	$indexlist_tpl = $tplDB->read_tpl("list_0.htm",$tpldb['list'],'index');
	...
	foreach($field_db AS $key=>$rs){
		...
		$tplDB->post_tpl($rs,$memberpost_tpl);	//前台发布页
		$tplDB->post_tpl($rs,$adminpost_tpl);	//后台发布页
	...
	$tplDB->write_tpl("post",$adminpost_tpl,'admin');
	$tplDB->write_tpl("post",$memberpost_tpl,'member');
	$tplDB->write_tpl("bencandy",$indexshow_tpl,'index');
	$tplDB->write_tpl("list",$indexlist_tpl,'index');
...
function check_field_name($id,$field_name){
	...
	if( table_field($this->table_title,$field_name)
		|| table_field($this->table_content,$field_name)){
		return true;
	}
}
...
function add_index($id,$field_name){
	global $db;
	$rs=$db->get_one("SHOW CREATE TABLE {$this->table_title}");
	if(!eregi("KEY `([_a-z0-9]+)` \(`{$field_name}`\)",$rs['Create Table'])){
		$db->query("ALTER TABLE `{$this->table_title}` ADD INDEX ( `{$field_name}` )");
	}
}
...
function creat_field($id,$postdb){
	...
	$this->table_add_field($this->table_title,$postdb[field_type],$postdb[field_name],$postdb[field_leng]);
...
function edit_field($id,$field_name,$postdb){
	...
	$this->table_edit_field($this->table_title,$postdb[field_type],$field_name,$postdb[field_name],$postdb[field_leng]);
...
function delete_field($id,$field_name){
	...
	$db->query("ALTER TABLE `{$this->table_title}` DROP `$field_name`");
