...
if($action=='mod'){
	...
	if(strstr($postdb[tplpart_2code],'$picurl')&&!strstr($postdb[tplpart_1code],'$picurl'))
	{
		if(strstr($postdb[tplpart_2code],'$content')){
			$SQL2=" SELECT A.*,A.aid AS id,R.content FROM {$_pre}article A LEFT JOIN {$_pre}reply R ON A.aid=R.aid $SQL AND A.ispic=1 AND R.topic=1 ORDER BY $order $asc LIMIT $sqlmin,1 ";
		}else{
			$SQL2=" SELECT A.*,A.aid AS id FROM {$_pre}article A $SQL AND A.ispic=1 ORDER BY $order $asc LIMIT $sqlmin,1 ";
		}		
	}
	...
	if( strstr($postdb[tplpart_1code],'$content')||strstr($postdb[tplpart_2code],'$content') ){
		$SQL=" SELECT A.*,A.aid AS id,R.content FROM {$_pre}article A LEFT JOIN {$_pre}reply R ON A.aid=R.aid  $SQL $SQLPIC AND R.topic=1 ORDER BY $order $asc LIMIT $sqlmin,$rows ";
	}else{
		$SQL=" SELECT A.*,A.aid AS id FROM {$_pre}article A $SQL $SQLPIC ORDER BY $order $asc LIMIT $sqlmin,$rows ";
	}
	
	$postdb[SYS]='CMS';
	$postdb['PRE']=str_replace('Info_','',$inc);
	$postdb[ctype]='article';
