...
if($action=='mod'){
	...
	if(strstr($postdb[tplpart_2code],'$picurl')&&!strstr($postdb[tplpart_1code],'$picurl'))
	{
		if(strstr($postdb[tplpart_2code],'$content')){
			$SQL2=" SELECT A.*,A.aid AS id,R.content FROM {$_pre}fu_article FA LEFT JOIN {$_pre}article A ON FA.aid=A.aid LEFT JOIN {$_pre}reply R ON A.aid=R.aid $SQL AND A.ispic=1 AND R.topic=1 ORDER BY $order $asc LIMIT $sqlmin,1 ";
		}else{
			$SQL2=" SELECT A.*,A.aid AS id FROM {$_pre}fu_article FA LEFT JOIN {$_pre}article A ON FA.aid=A.aid $SQL AND A.ispic=1 ORDER BY $order $asc LIMIT $sqlmin,1 ";
		}		
	}
	...
	if( strstr($postdb[tplpart_1code],'$content')||strstr($postdb[tplpart_2code],'$content') ){
		$SQL=" SELECT A.*,A.aid AS id,R.content FROM {$_pre}fu_article FA LEFT JOIN {$_pre}article A ON FA.aid=A.aid LEFT JOIN {$_pre}reply R ON A.aid=R.aid  $SQL $SQLPIC AND R.topic=1 ORDER BY $order $asc LIMIT $sqlmin,$rows ";
	}else{
		$SQL=" SELECT A.*,A.aid AS id FROM {$_pre}fu_article FA LEFT JOIN {$_pre}article A ON FA.aid=A.aid  $SQL $SQLPIC ORDER BY $order $asc LIMIT $sqlmin,$rows ";
	}
	
	$postdb[SYS]='CMS';
	$postdb['PRE']=str_replace('Info_','',$inc);
	$postdb[ctype]='fu_article';
	...
	$c_rolltypedb[$c_rolltype]=" checked ";
 	...
 	$select_news=$Guidedb->Checkbox("{$_pre}fu_sort",'fiddb[]',$fiddb);
