...
if(!$jobs&&$webdb[bencandy_cache_time]&&(time()-filemtime($Cache_FileName))>($webdb[bencandy_cache_time]*60)){
	...
	$content.="<SCRIPT LANGUAGE='JavaScript' src='$Mdomain/job.php?job=updatehits&aid=$id'></SCRIPT>";
...
function check_article(&$rsdb){
	...
	if($rsdb[passwd])
	{
		...
		else
		{
			...
			{
				echo "<CENTER><form name=\"form1\" method=\"post\" action=\"\">请输入软件密码:<input type=\"password\" name=\"password\"><input type=\"hidden\" name=\"TYPE\" value=\"article\"><input type=\"hidden\" name=\"NeedCheck\" value=\"1\"><input type=\"submit\" name=\"Submit\" value=\"提交\"></form></CENTER>";
				exit;
			}
		}
	}
	...
	if( $rsdb[money]=abs($rsdb[money]) ){
		...
		elseif( !strstr($rsdb[buyuser],",$lfjid,") )
		{
			...
			elseif($buy==1)
			{
				add_user($lfjuid,"-$rsdb[money]",'查看软件内容扣分');
				add_user($rsdb[uid],"$rsdb[money]",'软件被浏览奖分');
